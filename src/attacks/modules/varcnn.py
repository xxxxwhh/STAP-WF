...
class Encoder(nn.Module):
    """
    This class defines an encoder network composed of an initial convolutional block 
    followed by several dilated convolutional blocks.
    """
    def __init__(self):
...
class VarCNNNet(nn.Module):
    """
    VarCNN model for website fingerprinting attack.
    Uses dual encoders for directional and temporal features.
    
    Input format: [batch, 2, seq_length]
        - Channel 0: Direction sequence
        - Channel 1: Timing sequence
    """
    ...
    def forward(self, x):
        ...
        Args:
            x: [batch, 2, seq_length] tensor
        Returns:
            logits: [batch, num_classes] tensor
        """
        # Separate input into directional and temporal components
        x_dir = self.dir_encoder(x[:, 0:1, :])    # Direction encoder
        x_time = self.time_encoder(x[:, 1:, :])   # Timing encoder
